...
const ServerManagementPage: React.FC = () => {
  ...
  const handleResetPassword = async (server: StaffMember) => {
    ...
    const { data: hashedPassword, error: hashError } = await supabase.rpc('crypt', { password: newPassword, salt: genSalt(16) });
    ...
    <div className="container mx-auto p-6">
      ...
      {loading ? (
        ...
        <ul className="bg-white shadow rounded-lg divide-y divide-gray-200">
          {servers.map((server) => (
            <li key={server.id} className="p-4 flex items-center justify-between">
              <div>
